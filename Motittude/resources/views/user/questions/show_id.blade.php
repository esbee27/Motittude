...
<form action="{{ route('user.questions.update', $question->id) }}" method="POST">
    @csrf
    ...
    <div style="margin-top: 10px;">
        ...
        <textarea name="question_text" id="question_text" rows="4" cols="50" required>{{ $question->question_text }}</textarea>
    </div>
    ...
    <div style="margin-top: 10px;">
        ...
        @foreach ($question->options as $index => $option)
            ...
            <input type="radio" name="correct_answer" value="{{ $option->id }}" {{ $option->correct_answer ? 'checked' : '' }}> Correct
        @endforeach
